...
public class Stage5_BroomRightPUN : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        StartCoroutine(SweepBroomToRight());
        
    }

    public void Update()
    {
        if (PlayerControllerPUN.LocalPlayerInstanceLoadedFlagForBroomRight)
        {
            player = PlayerControllerPUN.LocalPlayerInstance;
    ...
    /*private void Update()
    {
        ...
        //Debug.Log(playerRigid2D.transform.position);
        collider2DHit = Physics2D.OverlapBox(rigid2D.transform.position, boxCollider2D.size, 0, 7); //7은 Player의 Layer
    ...
    IEnumerator MoveBroom()
    {
        ...
        {
            rigid2D.position = Vector2.Lerp(broomPosition, broomPosition + new Vector2(SweepLength, 0), curve.Evaluate(count / duration));
            count += 0.02f;
    ...
    IEnumerator SweepPlayer()
    {
        ...
        while (count < duration)
        {
            ...
            else
            {
                ...
                playerRigid2D.velocity += new Vector2(BroomPower, 0);
            }
