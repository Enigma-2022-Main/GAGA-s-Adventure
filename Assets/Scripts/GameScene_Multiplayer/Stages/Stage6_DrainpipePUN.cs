...
public class Stage6_DrainpipePUN : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "Player")
        {
            if (collision.gameObject.GetComponent<PhotonView>().IsMine)
            {
                ...
                if (PlayerControllerPUN.PlayerPosition.y < 120.7f)
                {
                    animationCounter = 2;
    ...
    IEnumerator DrainPipe_1()
    {
        while (animationCounter == 1)
        {
            ...
            PlayerControllerPUN.Stuck = true;
            PlayerControllerPUN.ForceLeft = true;
            if (PlayerControllerPUN.PlayerPosition.x <= 9f)
            {
    ...
    IEnumerator DrainPipe_2()
    {
        ...
        {
            if (PlayerControllerPUN.PlayerPosition.x <= 11.4f)
            {
    ...
    IEnumerator DrainPipe_3()
    {
        ...
        {
            if (PlayerControllerPUN.PlayerPosition.y < 21f)
            {
